...
func depthField(s interface{}, index interface{}) (interface{}, error) {
    ...
    case reflect.Map:
    	if sid.IsNil() {
     		return nil, fmt.Errorf("valexa: 该字段是 nil。错误的字段名为（%#v）", index)
  	 	}
    ...
    default:
    	return nil, fmt.Errorf("valexa: 非结构类型，无法正确读取。错误的类型为（%s）", sid.Kind())
    }
    if reflectValue.Kind() == reflect.Invalid {
    	return nil, fmt.Errorf("valexa: 该字段不是有效。错误的字段名为（%#v）", index)
    }
